...
//   try {
//     const res = await fetch("todo/clear", {
//       method: "DELETE"
//     });
...
// Select the button
const clearCompletedBtn = document.getElementById("clear-btn"); // check your HTML id

// Add click event
clearCompletedBtn.addEventListener("click", async () => {
  try {
    await axios.delete("http://localhost:4000/todo/clear");
    alert("All completed tasks cleared");
    getAllTodos(); // refresh UI after deletion
  } catch (error) {
    console.error("Error clearing tasks:", error);
    alert("Something went wrong while clearing tasks");
  }
});
